...
static unsigned char *make_utf8_string(const wchar_t *unicode)
{
    ...
    while(c)
    {
        if(c < 0x0080) {
            ...
            out[out_index++] = 0xc0 | (c >> 6); //high 5 bits
            out[out_index++] = 0x80 | (c & 0x3f); //low 6 bits
        } else {
            out[out_index++] = 0xe0 | (c >> 12); //high 4 bits
            out[out_index++] = 0x80 | ((c >> 6) & 0x3f); //middle 6 bits
            out[out_index++] = 0x80 | (c & 0x3f); //low 6 bits
        }
        c = unicode[index++];
    }
...
/**	the aurthor's code doesn't work on my linux, 
 *	so i write one for myself... 
 *				echo<echo.xjtu@gmail.com>
 */
#ifndef USE_DEFAULT_UTF8_C	
#include <iconv.h>
int utf8_encode(const char *from, char **to)
{
	iconv_t cd;

	int inlen = strlen(from);
	int outlen = inlen*3;
	...
	if(-1 == iconv(cd,(char**)&from,(size_t *)&inlen, &pout,(size_t *)&outlen) ) {
		free(outbuf);
		iconv_close(cd); 
		return -1;
	} else {
		outlen = oldoutlen - outlen;
		*to=(char*)malloc(outlen+1);
		strncpy(*to,outbuf,outlen);
		(*to)[outlen] = '\0';
		free(outbuf);
		iconv_close(cd); 
		return 0;
...
static char *current_charset = "GBK"; /* means "US-ASCII" */
